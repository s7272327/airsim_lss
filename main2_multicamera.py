...
scene_name = "downtown_west_0-1-test"

#加载数据
with open(f"pkls/drones_states_{scene_name}.pkl", "rb") as f:
...
def remove_physics_engine_line(filepath):
    with open(filepath, 'r', encoding='utf-8') as f:
        lines = f.readlines()

    with open(filepath, 'w', encoding='utf-8') as f:
        for line in lines:
            # 去除包含 PhysicsEngineName 的行
            if '"PhysicsEngineName"' not in line:
                f.write(line)
    print("已恢复有中立引擎模式")
...
def main():
    #读取settings.json
    settings_path = os.path.expanduser("C:/Users/sxk/Documents/AirSim/settings.json")  # 改成实际路径
    ...
    #去除engine,回复有引擎模式
    remove_physics_engine_line(settings_path)
